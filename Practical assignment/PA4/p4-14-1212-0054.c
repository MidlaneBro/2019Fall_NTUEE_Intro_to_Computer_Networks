    ...
    while(1) {  // main accept() loop
        ...
        if(strcmp(pch1,"GET")==0){        //Check whether the message is a GET message
            ...
            if(requested_file!=NULL){  //Check if the file, </path/name>, exists
                stringlen = sprintf(tmpstring,"HTTP/1.0 200 Document Follows\n");
                ...
                rewind(requested_file);                   // move to the beginning of the file
                stringlen=sprintf (tmpstring, "Content-Length: %d\n",end);
                printf("Content-Length: %d\n",end);
                send(new_fd, tmpstring, stringlen, 0);
                stringlen=sprintf (tmpstring, "\n");
                send(new_fd, tmpstring, stringlen, 0);
                char buffer[1024];
                while(end>1024){
                    printf("left %d\n",end);
                ...
                stringlen=fread(buffer,1,end,requested_file);
                send(new_fd,buffer,stringlen,0);
            }
